...
source: "https://github.com/twigphp/Twig/blob/ecb310e129f549daa3424f6565520bcc807d16f3/doc/templates.rst"
upstreamPath: "doc/templates.rst"
editLink: true
vue: false
---
:::::: v-pre
...
> **To experiment with Twig, you can use the [Twig Playground](https://twig.symfony.com/play).**
>
>
...
* *Textmate* via the [Twig bundle](https://github.com/uhnomoli/PHP-Twig.tmbundle)
* *Vim* via the [vim-twig plugin](https://github.com/lumiliet/vim-twig)
* *Netbeans* (native as of 7.2)
* *PhpStorm* (native as of 2.1)
* *Eclipse* via the [Twig plugin](https://github.com/pulse00/Twig-Eclipse-Plugin)
* *Sublime Text* via the [Twig bundle](https://github.com/uhnomoli/PHP-Twig.tmbundle)
...
* *Emacs* via [web-mode.el](https://web-mode.org/)
* *Atom* via the [PHP-twig for atom](https://github.com/reesef/php-twig)
* *Visual Studio Code* via the [Twig pack](https://marketplace.visualstudio.com/items?itemName=bajdzis.vscode-twig-pack), [Modern Twig](https://marketplace.visualstudio.com/items?itemName=Stanislav.vscode-twig) or [Twiggy](https://marketplace.visualstudio.com/items?itemName=moetelo.twiggy)

You might also be interested in:

* [Twig CS Fixer](https://github.com/VincentLanglet/Twig-CS-Fixer): a tool to check/fix your templates code style
* [Twig Language Server](https://github.com/kaermorchen/twig-language-server/tree/master/packages/language-server): provides some language features like syntax
  highlighting, diagnostics, auto complete, ...
* [TwigQI](https://github.com/alisqi/TwigQI): an extension which analyzes your templates for common bugs during compilation
* [TwigStan](https://github.com/twigstan/twigstan): a static analyzer for Twig templates powered by PHPStan

## Variables

Twig templates have access to variables provided by the PHP application and
variables created in templates via the [set](tags/set) tag. These
...
`strict_variables` option value (see `environment options`):
...
[set](tags/set) tag or the `= operator`:
...
See the [set](tags/set) tag documentation for more details.
...
Go to the [filters](/3.x/#filters) page to learn more about built-in
filters.


> **As the `filter` operator has the highest `precedence`, use parentheses when filtering more "complex"**
>
> expressions:
>
>
...
Go to the [functions](/3.x/#functions) page to learn more about the
built-in functions.
...
`users`, use the [for](tags/for) tag:
...
Go to the [tags](/3.x/#tags) page to learn more about the built-in tags.
...
The [include](functions/include) function is useful to include a template
...
In this example, the [block](tags/block) tags define four blocks that
...
The [extends](tags/extends) tag is the key here. It tells the template
...
[parent](functions/parent) function. This gives back the results of the
parent block:
...
> **The documentation page for the [extends](tags/extends) tag describes**
>
...
`autoescape` option and defaults to `html`.
...
template to be escaped or not by using the [autoescape](tags/autoescape)
tag:
...
[macro](tags/macro) tag documentation.
...
* `//`: Divides two numbers and returns the floored integer result. ``&#123;&#123; 20
  ...
  sugar for the [round](filters/round) filter).
...
> **Read more about in the [PHP spaceship operator documentation](https://www.php.net/manual/en/language.operators.comparison.php).**
>
>
...
>
> [regular expressions](https://www.php.net/manual/en/pcre.pattern.php):
>
>
> ```twig
> {% if phone matches '/^[\\d\\.]+$/' %}
...
Go to the [tests](/3.x/#tests) page to learn more about the built-in
tests.
...
* `..`: Creates a sequence based on the operand before and after the operator
  (this is syntactic sugar for the [range](functions/range) function):
...
```
  ...
  Before Twig 3.15, use the [attribute](functions/attribute) function
...
```
  ...
  The assignment operator also supports `destructuring`.
...
>
  ...
  Arrow functions can be called using the [invoke](filters/invoke)
  filter.
...
variables in a single operation using the `=` `assignment operator`.
...
> **Object destructuring uses the `dot operator` to access**
>
